...
export const ResizableWidget: FC<AppProps> = ({ dashboardOid, widget, themeSettings, index, onWidgetDelete, onWidgetOrderUpdate }) => {
    const [key, setKey] = useState<string>(`movableWidget ${index} ${widget.oid}`);
    ...
    return (
        <StyledResizableBlock
            ...
            resizeHandles={['se']}
        >   <>
                <StyledLeftButton onClick={() => onWidgetOrderUpdate(index, index - 1)} aria-label="order-left">
                    <LeftIcon />
                </StyledLeftButton>
                <StyledRightButton onClick={() => onWidgetOrderUpdate(index, index + 1)} aria-label="order-right">
                    <RightIcon />
                </StyledRightButton>
                <StyledDeleteButton onClick={() => onWidgetDelete(widget)} aria-label="delete">
                    <DeleteIcon />
                </StyledDeleteButton>
                <DashboardWidget
                    key={key}
